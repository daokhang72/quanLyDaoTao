...
export async function editHocPhan(hocPhan, ctdtId) {
    /* ------------------- Lấy danh sách khung ------------------- */
    let khungList = [];
    try {
        khungList = await getKhungByCTDTId(ctdtId);
    } catch (err) {
        console.error('Lỗi lấy khung:', err);
        alert('Không lấy được danh sách khung. Vui lòng thử lại!');
        return;
    ...
    /* ------------------- Tạo HTML form ------------------- */
    modal.innerHTML = `
    <h2 style="margin-top:0">Sửa Học Phần</h2>
    <form id="hocphan-form">
      <div style="margin-bottom:12px">
        <label>Khung <span style="color:red">*</span></label><br/>
        <select name="khungId" required style="width:100%;padding:6px"></select>
      ...
      <div style="text-align:right">
        <button type="submit" style="padding:6px 14px">Lưu</button>
        <button type="button" id="hp-cancel" style="padding:6px 14px;margin-left:8px">Hủy</button>
      </div>
    </form>
  `;

    const form = modal.querySelector('#hocphan-form');

    /* ------------------- Gắn option khung vào select ------------------- */
    const selectEl = form.querySelector('select[name="khungId"]');
    khungList.forEach(k => {
        const opt = document.createElement('option');
        opt.value = k.khungId;
        opt.textContent = k.tenNhom || `Khung ${k.id}`;
        if (k.khungId === hocPhan.khungId) opt.selected = true;
        selectEl.appendChild(opt);
    });

    /* ------------------- Đặt giá trị hiện tại vào form ------------------- */
    form.tenHocPhan.value = hocPhan.tenHocPhan;
    form.soTinChi.value = hocPhan.soTinChi ?? '';
    form.soTietLyThuyet.value = hocPhan.soTietLyThuyet ?? '';
    form.soTietThucHanh.value = hocPhan.soTietThucHanh ?? '';
    form.soTietThucTap.value = hocPhan.soTietThucTap ?? '';
    form.heSoHocPhan.value = hocPhan.heSoHocPhan ?? '';
    ...
    form.addEventListener('submit', async e => {
        ...
        // Chuyển về dạng số nếu có
        ['khungId', 'soTinChi', 'soTietLyThuyet', 'soTietThucHanh', 'soTietThucTap', 'heSoHocPhan']
            .forEach(k => data[k] = data[k] === '' ? null : Number(data[k]));

        try {
            console.log('Payload cập nhật:', data);
            await updateHocPhanById(hocPhan.maHp, data); // cần có API này
            alert('Cập nhật học phần thành công!');
            loadHP(ctdtId); // reload danh sách
            close();
        } catch (err) {
